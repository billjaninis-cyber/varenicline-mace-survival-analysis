...
# - Cluster-robust inference in the matched cohort
#
...
############################################################
# 5a. MISSINGNESS CHECK (new)
############################################################

# Report how many rows are incomplete on the analysis variables
# before any model silently drops them via na.action = na.omit

n_incomplete <- sum(!complete.cases(d1[, c(all_vars, "treatment", "survival_time", "MACE")]))

cat(
  "Rows with missing data on analysis variables: ", n_incomplete,
  " of ", nrow(d1), " (", round(100 * n_incomplete / nrow(d1), 2), "%)\n",
  sep = ""
)


############################################################
# 5b. RACE CATEGORY / SEPARATION CHECK (new)
############################################################

# Check for sparse Race categories that are near-empty within a
# treatment arm; these will cause quasi-complete separation
# (huge coefficient, huge SE, uninterpretable CI) in the PS and
# adjusted Cox models below.

race_by_treatment <- table(d1$Race, d1$treatment_label)
...
sparse_race_flag <- apply(race_by_treatment, 1, function(x) any(x < sparse_threshold))

if (any(sparse_race_flag)) {
  cat(
    "\nWARNING: the following Race level(s) have fewer than",
    sparse_threshold, "patients in at least one treatment arm and are\n",
    "at risk of quasi-complete separation in the PS/adjusted Cox models:\n"
  )
  print(rownames(race_by_treatment)[sparse_race_flag])
  cat(
    "Consider collapsing sparse Race levels into an 'Other' category\n",
    "before fitting the PS and adjusted Cox models below.\n\n"
  )
}

# Collapse sparse Race levels into "Other" so the PS/Cox models below
# do not silently produce a separated, uninterpretable coefficient.
# Comment this block out if the sparse level is intentional/required
# by the assignment.

d1$Race_collapsed <- as.character(d1$Race)
sparse_levels <- rownames(race_by_treatment)[sparse_race_flag]
d1$Race_collapsed[d1$Race_collapsed %in% sparse_levels] <- "Other"
d1$Race_collapsed <- factor(d1$Race_collapsed)

cat("Race_collapsed levels used in models:\n")
print(table(d1$Race_collapsed, d1$treatment_label))
...
# Label rows explicitly (rowsum() output order follows factor coding,
# not a guaranteed label match) so the printed table is unambiguous.

rownames(incidence_rates) <- levels(d1$treatment_label)
...
# Uses Race_collapsed (see section 5b) to avoid separation from
# sparse Race levels.
...
# (Race_collapsed used here too, for the same reason as section 10)
...
# Report matched vs. unmatched/discarded sample sizes explicitly (new).
# The matched HR generalizes to the matched sample, not the full
# cohort -- worth stating in the writeup.

match_nn <- summary(match_model)$nn

cat("\nMatching sample sizes (All / Matched / Unmatched / Discarded):\n")
print(match_nn)
...
# Cluster-robust (pair-aware) standard errors (new). Matched pairs
# are not independent observations, so the naive SE below understates
# uncertainty; `cluster = subclass` corrects for this using the
# matched-pair ID that MatchIt assigns in match.data().

matched_data$subclass <- factor(matched_data$subclass)
...
############################################################
# 20a. E-VALUE FOR THE MATCHED (PRIMARY) ESTIMATE (new)
############################################################

# Quick sensitivity check for unmeasured confounding on the matched
# HR. Manual E-value calculation (Vanderweele & Ding, 2017) so no
# additional package (e.g. EValue) is required.

e_value <- function(hr) {
  rr <- if (hr < 1) 1 / hr else hr
  rr + sqrt(rr * (rr - 1))
}

matched_hr <- matched_results["treatment", "HR"]
matched_ci_bound <- ifelse(
  matched_hr < 1,
  matched_results["treatment", "X97.5.."],
  matched_results["treatment", "X2.5.."]
)

cat(
  "\nE-value for matched HR (", round(matched_hr, 3), "): ",
  round(e_value(matched_hr), 2),
  "\nE-value for CI bound closer to the null (", round(matched_ci_bound, 3), "): ",
  round(e_value(matched_ci_bound), 2),
  "\n",
  sep = ""
)
